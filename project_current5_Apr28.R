...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    tags$div(style = "padding: 0 12px;",
      ...
      numericInput("cp_hitpct",  "Hit Efficiency",        value = NA, step = 0.001),
...
server <- function(input, output, session) {
  ...
  output$similar_player <- renderUI({
    ...
    if (is.null(ps) || is.na(arch)) {
      return(tags$p(style = "color:#aaa; font-style:italic;",
                    "Most similar player will appear here."))
    }

    vars <- position_vars[[pos]]
    ps_vec <- unlist(ps)[vars]
    ...
    if (nrow(df_pos) == 0) {
      return(tags$p("No similar players found."))
    }

    # Euclidean distance to each player
    dists <- apply(df_pos[, vars], 1, function(row) {
      sqrt(sum((row - ps_vec)^2, na.rm = TRUE))
    })

    best <- df_pos[which.min(dists), ]

    tags$div(
      tags$p(style = "font-size:13px; color:#aaa; margin-bottom:4px;",
             "Player is most similar to:"),
  ...
  output$radar_plot <- renderPlotly({
    ...
    scale_vs_group <- function(vals) {
      sapply(seq_along(vars), function(i) {
        ...
        hi  <- quantile(comp_df[[vars[i]]], 0.95, na.rm = TRUE)
        if (hi == lo) 0.5
